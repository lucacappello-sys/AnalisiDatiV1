...
def start_recording(filename):
    ...
    print(f"REGISTRAZIONE INIZIATA: {full_filename}")
...
def run_tracking(session_name="yolo_session"):
    app_state["session_name"] = session_name
    ...
    cv2.namedWindow('YOLOv11 RULA ENGINE')
    ...
    try:
        ...
        while True:
            ...
            # --- MODIFICA CRUCIALE PER EVITARE L'INDEXERROR ---
            if len(results) > 0 and results.keypoints is not None:
                # Esegue i calcoli solo se c'è almeno un rilevamento
                ...
                if app_state["recording"] and len(full_3d) > 12:
                    ...
                    tr_ang, nk_ang, ua_ang, la_ang = calculate_inclination(hip, sh), calculate_inclination(sh, ear), calculate_inclination(sh, el), calculate_angle(sh, el, wr)
                ...
                # Feedback a video se nessuno è rilevato
                cv2.putText(img, "SOGGETTO NON RILEVATO", (180, 240), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
            ...
            if app_state["recording"]:
                ...
                cv2.putText(img, "READY - STANDBY (R to Record)", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)

            cv2.imshow('YOLOv11 RULA ENGINE', img)
            
            key = cv2.waitKey(1)
            if key == 27: break # ESC
            elif key == ord('r') or key == ord('R'):
                if not app_state["recording"]: start_recording(app_state["session_name"])
                else: stop_recording()
